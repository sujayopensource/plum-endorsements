...
- [10. Production Upgrade Path: Rule-Based to ML/LLM](#10-production-upgrade-path-rule-based-to-mlllm)
  - [10.1 The Swap](#101-the-swap)
  - [10.2 Technology Stack for ML/LLM Upgrade](#102-technology-stack-for-mlllm-upgrade)
...
- [14. Three-Tier Evolution Strategy](#14-three-tier-evolution-strategy)
...
This document is the **architectural foundation** for AI/automation in the Plum Endorsement system. Two companion documents provide actionable implementation details:
...
| **[AI Automation Implementation Approach](AI_Automation_Implementation_Approach.md)** | Enhancements implementable **now** with existing stack (Java 21, Spring Boot, PostgreSQL, Redis) | 24 items across 4 phases | ~34 dev-days |
| **[AI Automation Vision](AI_Automation_Vision.md)** | Future ML/AI capabilities requiring new infrastructure (Python sidecars, LLM APIs, vector DBs) | 15 capabilities across 5 pillars | 6-12 months |

**Reading order:**
1. **This document** -- understand the architecture and current algorithms
2. **Implementation Approach** -- what to build in the next 6-8 weeks
3. **Vision** -- the long-term AI/ML roadmap
...
|                                                                   |
|  TODAY (Rule-Based)              FUTURE (ML/LLM-Backed)           |
|  +----------------------+        +----------------------+         |
|  | RuleBasedAnomaly     |   -->  | IsolationForest      |         |
|  | Detector             |        | AnomalyDetector      |         |
|  +----------------------+        +----------------------+         |
|  +----------------------+        +----------------------+         |
|  | StatisticalForecast  |   -->  | ProphetForecast      |         |
|  | Engine               |        | Engine               |         |
|  +----------------------+        +----------------------+         |
|  +----------------------+        +----------------------+         |
|  | SimulatedError       |   -->  | RAGErrorResolver     |         |
|  | Resolver             |        | (LangChain4j)        |         |
|  +----------------------+        +----------------------+         |
|  +----------------------+        +----------------------+         |
|  | EventStream          |   -->  | PM4PyProcess         |         |
|  | Analyzer             |        | Analyzer             |         |
|  +----------------------+        +----------------------+         |
|  +----------------------+        +----------------------+         |
|  | ConstraintBatch      |   -->  | ORToolsBatch         |         |
|  | Optimizer            |        | Optimizer            |         |
|  +----------------------+        +----------------------+         |
+------------------------------------------------------------------+
```
...
Each port defines a single method with domain-level input/output records:

| Port Interface | Current Adapter | Method Signature |
|---|---|---|
| `AnomalyDetectionPort` | `RuleBasedAnomalyDetector` | `analyzeEndorsement(Endorsement, List<Endorsement>)` --> `AnomalyResult` |
...
| `ErrorResolutionPort` | `SimulatedErrorResolver` | `analyzeError(Endorsement, String errorMessage, UUID insurerId)` --> `ResolutionSuggestion` |
...
```
  ...
  (no change)                            (no change)
       |                                      |
       v                                      v
  AnomalyDetectionPort                  AnomalyDetectionPort
       |                                      |
       v                                      v
  RuleBasedAnomalyDetector    ===>    IsolationForestDetector
  (deterministic, fast)               (ML-backed, Spring AI)
  ...
  Zero changes to:
    ...
    - REST controllers
    - Schedulers
    - Tests (port-level mocks still work)
```

The `@Component` and `@Primary` annotations from Spring's dependency injection handle the swap. Feature flags via `@ConditionalOnProperty` allow gradual rollout.
...
Four rule functions run in parallel, and the **highest-scoring** rule wins:
...
Future:    IsolationForestDetector (via Spring AI)
...
Migration: 1. Collect 90 days of labeled anomaly data (FLAGGED -> CONFIRMED_FRAUD)
           2. Train Isolation Forest on historical endorsement features
           3. Deploy as new @Component @Primary adapter
           4. Run shadow mode: both adapters run, compare results
           5. Promote ML adapter when precision/recall meet targets
```
...
Future:    ProphetForecastEngine (via Spring AI)
...
Migration: 1. Accumulate 6+ months of daily EA transaction history
           2. Train Prophet model per (employerId, insurerId) pair
           3. Validate against StatisticalForecastEngine predictions
           4. Deploy as @Primary adapter with A/B testing
```
...
Future:    RAGErrorResolver (via LangChain4j)
...
Thirteen endpoints are exposed under `/api/v1/intelligence/`, served by the `IntelligenceController`.
...
## 10. Production Upgrade Path: Rule-Based to ML/LLM

> **Detailed implementation guidance:**
> - For enhancements within the current rule-based system, see **[AI Automation Implementation Approach](AI_Automation_Implementation_Approach.md)** (24 items, Java-only)
> - For the ML/LLM adapter swap path, see **[AI Automation Vision](AI_Automation_Vision.md)** (15 capabilities, new infrastructure)

### 10.1 The Swap

The key architectural insight is that **upgrading from rule-based to ML/LLM-backed intelligence requires zero changes to domain or application code**. Only new adapter implementations in the infrastructure layer:
...
```
  ...
  AnomalyDetectionPort                  AnomalyDetectionPort
       ...
       v                                      v
  @Component                             @Component
  RuleBasedAnomalyDetector              RuleBasedAnomalyDetector
  (still deployed as fallback)          (fallback, lower priority)
                                              |
                                              v
                                         @Component @Primary
                                         IsolationForestDetector
                                         (ML-backed, takes priority)
```

### 10.2 Technology Stack for ML/LLM Upgrade
...
```
  ...
  +-----------------------------------+
  | Both adapters run simultaneously  |
  ...
  | Rule-based as automatic fallback  |
  | Circuit breaker on ML adapter     |
  +-----------------------------------+
           |
           v
  Phase 4: Full Migration
  +-----------------------------------+
  | ML adapter is @Primary            |
  | Rule-based adapter remains for    |
...
|  Current:       5 rule-based adapters (deterministic, auditable)   |
|  Future:        ML/LLM adapters via Spring AI + LangChain4j       |
|  Migration:     Shadow mode -> canary -> graduated -> full         |
|                                                                    |
|  +-------------------+-------------------+-----------------------+ |
|  | Pillar            | Schedule          | Key Metric            | |
|  +-------------------+-------------------+-----------------------+ |
|  | Anomaly Detection | Every 5 min       | anomaly.detected rate | |
|  | Balance Forecast  | Daily 6 AM        | shortfall.detected    | |
|  | Error Resolution  | On rejection      | auto-resolve rate %   | |
|  | Process Mining    | Daily 3 AM        | STP rate per insurer  | |
|  | Batch Optimizer   | Every 15 min      | savings per batch Rs. | |
|  +-------------------+-------------------+-----------------------+ |
|                                                                    |
|  Events:  6 intelligence event types --> Kafka                     |
|  API:     13 REST endpoints under /api/v1/intelligence/            |
|  Metrics: 12+ Micrometer metrics --> Prometheus --> Grafana         |
|                                                                    |
|  Key insight: We do not need ML to deliver intelligence.           |
|  We need ML to deliver BETTER intelligence -- and the              |
|  architecture ensures that upgrade is a deployment-time            |
|  adapter swap, not an application rewrite.                         |
+-------------------------------------------------------------------+
```

---

## 14. Three-Tier Evolution Strategy

The AI/automation strategy is organized into three tiers, each documented separately:

```
+-------------------------------------------------------------------+
|                    THREE-TIER INTELLIGENCE EVOLUTION                |
+-------------------------------------------------------------------+
|                                                                    |
|  TIER 1: CURRENT STATE (This Document)                            |
|  ======================================                            |
|  5 rule-based adapters, fully operational                          |
|  4 anomaly rules, linear forecasting, keyword error matching       |
|  Event pair process mining, 0-1 knapsack batch optimization        |
|  13 REST endpoints, 6 Kafka event types, 12+ Prometheus metrics    |
|  Status: DEPLOYED AND TESTED (762 tests, all passing)              |
|                                                                    |
...
+-------------------------------------------------------------------+
|                                                                    |
|  The three tiers are ADDITIVE, not SEQUENTIAL.                    |
|  Tier 2 enhancements improve the rule-based system.               |
|  Tier 3 ML adapters replace the rule-based system.                |
|  Both paths are enabled by the port/adapter architecture.         |
|                                                                    |
+-------------------------------------------------------------------+
```
...
Intelligence Source Code:
  ...
  1 REST controller (13 endpoints)
  14 DTOs (request/response records)
  7 domain models (entities + enums)
  4 Flyway migrations (anomaly, forecasts, errors, process mining)

Intelligence Test Coverage:
  Unit tests:        Testing all 5 adapters, 5 services, 8 schedulers, 1 controller
...
|----------|---------|-------------|
| [AI Automation Implementation Approach](AI_Automation_Implementation_Approach.md) | 24 near-term enhancements | Feedback loops, enhanced rules, operational improvements, rich narratives |
| [AI Automation Vision](AI_Automation_Vision.md) | 15 future ML/AI capabilities | Isolation Forest, Prophet, RAG+LLM, PM4Py, OR-Tools, MLOps, Feature Store |
